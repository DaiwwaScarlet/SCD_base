...
# 数据集
from dataset import second as se

# 网络
from models.test import SepNet as Net
...
args = {
    ...
    'gpu': True,
    'epochs': 50,
    'lr_decay_power': 1.5, # 学习率衰减的指数幂 1.5
    'psd_train': True, # 伪标签训练
    'psd_TTA': True, # 使用伪标签训练时进行测试时数据增强
    'vis_psd': True, # 可视化伪标签结果
    'psd_init_Fscd': 0.6, # 启用伪标签训练的 Fscd 指标初始阈值
    'print_freq': 50, # 打印频率
    'predict_step': 5, # 多少epoch预测结果
    'pseudo_thred': 0.6, # 伪标签阈值，过滤低置信度的预测
...
# 根据预测结果，为每个类别计算更新的置信度阈值
# 返回bool矩阵（置信度） index每个像素的预测类别
def calc_conf(prob, conf_thred):
    b, c, h, w = prob.size()
    conf, index = torch.max(prob, dim=1)
    index_onehot = F.one_hot(index.long(), num_classes=se.num_classes).permute((0, 3, 1, 2))
...
def main():
    # 模型
    net = Net(3, num_classes=se.num_classes).cuda()
    ...
    # shuffle
    train_set = se.Data('train', random_flip=True, random_swap=False)
...
# 输入： dataloader net 损失 优化器 val_loader
def train(train_loader, net, criterion, optimizer, val_loader):
    net_psd = None
    conf_thred = AverageThred(se.num_classes)
    ...
    all_iters = float(len(train_loader) * args['epochs']) # 迭代次数
    criterion_sc = ChangeSimilarity().cuda()  # 余弦相似度
    curr_epoch = 0
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            assert outputs_A.size()[1] == se.num_classes
            ...
            if args['psd_train'] and bestFscdV > args['psd_init_Fscd']:
                ...
                if args['vis_psd'] and not running_iter % 100:
                    psdA_color = se.Index2Color(labels_A[0].cpu().detach().numpy())
                    psdB_color = se.Index2Color(labels_B[0].cpu().detach().numpy())
                    io.imsave(os.path.join(args['pred_dir'], NET_NAME + imgs_id[0] + '_psdA_epoch%diter%d.png' % (
                    curr_epoch, running_iter)), psdA_color)
                    io.imsave(os.path.join(args['pred_dir'], NET_NAME + imgs_id[0] + '_psdB_epoch%diter%d.png' % (
                    curr_epoch, running_iter)), psdB_color)
...
def validate(val_loader, net, criterion, curr_epoch):
    ...
    for vi, data in enumerate(val_loader):
        ...
        if curr_epoch % args['predict_step'] == 0 and vi == 0:
            pred_A_color = se.Index2Color(preds_A[0])
            pred_B_color = se.Index2Color(preds_B[0])
    ...
    Fscd, IoU_mean, Sek = SCDD_eval_all(preds_all, labels_all, se.num_classes)
