...
输入：img（图像数组）
输出：0（返回值仅用于结束函数）
作用：使用 matplotlib 显示输入图像。

5. rand_flip_SCD
输入：img1（图像1），img2（图像2），label1（标签1），label2（标签2）
输出：翻转后的图像1、图像2、标签1、标签2
作用：对变化检测任务中的两张图像和其标签进行随机翻转。

6. rand_flip_SCD5
输入：img1，img2，label1，label2，label3，label4
输出：翻转后的图像和标签
作用：对4个标签和两张图像进行随机翻转。

7. rand_rot90_SCD
输入：img1（图像1），img2（图像2），label1（标签1），label2（标签2）
输出：旋转后的图像1、图像2、标签1、标签2
作用：以 90 度为基准，对两张图像及其标签进行随机旋转。

8. rand_rot90_SCD5
输入：img1，img2，label1，label2，label3，label4
输出：旋转后的图像和标签
作用：对两张图像及4个标签进行随机 90 度旋转。

9. rand_rot90_flip_SCD
输入：img1（图像1），img2（图像2），label1（标签1），label2（标签2）
输出：同时旋转和翻转后的图像和标签
作用：对图像和标签先进行 90 度旋转，再进行随机翻转。

10. rand_rot90_flip_SCD5
输入：img1，img2，label1，label2，label3，label4
输出：旋转和翻转后的图像和标签
作用：对两张图像和4个标签进行 90 度旋转并进行翻转。
'''
