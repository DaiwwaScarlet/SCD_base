...
# 为传入的模型（如卷积层和线性层）初始化权重，
# 采用 Kaiming 正态分布进行初始化 特别适合具有 ReLU 激活函数的深度神经网络
# BatchNorm 层设置权重为 1，偏置为 0。
# 有两个可学习的参数：权重（weight）和偏置（bias）
...
# 多项式学习率衰减
# 初始化：optimizer优化器（adam SGD） 当前迭代次数 最大迭代次数 衰减因子
# param_groups 引用优化器中的参数组
# 通过逐步降低学习率来提高模型的泛化能力
class PolyLR(object):
